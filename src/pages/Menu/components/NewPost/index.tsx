...
export function NewPost() {
    ...
    function handleCreateNewPost(data: NewPostFormInputs) {
        const verifyPostEqualTitle = posts.findIndex(post => post.title === data.title)
    ...
    return (
        <DialogContent>
            ...
            <form className="flex flex-col gap-2" onSubmit={handleSubmit(handleCreateNewPost)}>
                <Input
                    type="text"
                    placeholder="Título"
                    required
                ...
                <Textarea
                    ...
                    {...register('body')}
                />
                <Button type="submit" className="font-extrabold">Publique</Button>
            </form>
        </DialogContent>
    )
}